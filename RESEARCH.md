# How Flowgate Lets Commodity LLMs Outperform Frontier Models on Software Engineering
...
work, at a fraction of the cost. **`mcp-flowgate` is that gateway.** This
document explains the evidence, the architecture, and how to build a
commodity-LLM coding agent on top of Flowgate today.
...
deterministic control plane around narrowly-scoped model roles. Flowgate
...
## How Flowgate decomposes the actor model
...
is a separate sub-system. Each decomposes cleanly into Flowgate primitives
that already exist:

| Actor | Flowgate state | Flowgate guidance (skill) | Flowgate executor |
|---|---|---|---|
...
**Six of seven actors are expressible today** with the primitives Flowgate
already ships. The seventh (Router) disappears — routing is what Flowgate
...
Flowgate crate, plugging in via the existing `connections:` proxy model.

---

## Six design principles

| Principle | What it means | Flowgate primitive |
|---|---|---|
...
| **Just-in-time retrieval** | Load only the graph slice, files, docs relevant to the current hypothesis | HATEOAS surfacing of guidance refs; `gateway.describe` on demand (SPEC §5.4, §5.7) |
...
architecture — converges on these six principles. Flowgate's design
...
## Concept of operations as a Flowgate workflow
...
## Reference workflow (Flowgate YAML)
...
no agent loop, no scheduler. Flowgate's existing runtime walks the state
...
Flowgate enforces these invariants by construction, not by prompt:
...
## Typed artifacts: from TypeScript intent to Flowgate reality

RESEARCH literature often expresses the protocol as TypeScript interfaces.
In Flowgate, those interfaces become **Rust types in the codebase** plus
**JSON schemas at the wire**:

| Research-paper type | Flowgate equivalent |
|---|---|
| `Envelope<T>` (traceId, runId, messageId, actor, …) | `AuditEvent` in `crates/mcp-flowgate-core/src/audit.rs` (workflow_id, correlation_id, actor, event_type, payload). **Add traceId/runId** per §20.2 below. |
...
Flowgate's role is to **invoke** the harness deterministically, capture its
...
| Check layer | Purpose | Flowgate manifestation |
|---|---|---|
...
shows up as a distinctive pattern in Flowgate's transition records:
...
Three categories of gap remain. None requires changes to Flowgate's
...
These are not Flowgate code. They are external systems that the
...
- `examples/swe-agent.yaml` — the workflow YAML above, with `connections:`
...
### Gap C: Minor Flowgate enrichments (light Rust additions)
...
Extend `crates/mcp-flowgate-core/src/model.rs::Evidence` with two optional
fields:
...
Extend `crates/mcp-flowgate-core/src/audit.rs::AuditEvent` with two
...
Three RESEARCH topics belong outside Flowgate by design:

- **Sandbox selection (gVisor vs Firecracker).** Configured in the verifier
  harness's container runtime. Flowgate is unaware of container internals.
- **Supply chain attestation (Sigstore, SLSA).** CI/CD pipeline integration.
  Flowgate records that the build stage ran and produced an attestation
  artifact; the attestation itself is the CI's product.
- **Model-tier selection at runtime.** The LLM caller's routing layer
  decides which model handles each tool call. Flowgate provides the audit
  record from which the caller learns *which* tier worked; choosing is the
  caller's domain.

These boundaries are intentional. Pushing them into Flowgate would couple
...
| Product-specific | Local workflows, editor/CLI behaviour | Internal Flowgate workflow fixtures (the e2e suite already in `crates/mcp-flowgate-mcp-server/tests/`) |
...
The Flowgate architecture compensates for each:
...
research base is thinner for non-Python codebases. Flowgate's
...
authoritative acceptance gate. Flowgate's `evidence` guard with multi-stage
...
The Flowgate audit log provides every input a learning estimator would
...
`mcp-flowgate`. What remains is workflow authoring, skill writing, and
...
Plus the `mcp-flowgate` SPEC sections referenced throughout (§5–§19).
