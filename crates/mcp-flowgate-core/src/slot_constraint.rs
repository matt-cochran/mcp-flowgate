...
pub fn evaluate_constraints(
    ...
    let state_slots = definition
        .pointer(&format!(
            "/states/{}/slots",
            crate::runtime_links::pointer_escape(state)
        ))
...
fn build_glob_set(
    ...
    if let Some(arr) = list.and_then(Value::as_array) {
        for (i, pat) in arr.iter().enumerate() {
            ...
            let glob = Glob::new(s).map_err(|e| ConstraintViolation {
                slot: slot.to_string(),
                constraint_kind: "path_allowlist".into(),
                message: format!(
                    "SLOT_CONSTRAINT_VIOLATED: slot '{slot}' constraint.path_allowlist.{field}[{i}] \
                     '{s}' is not a valid glob pattern: {e}"
                ),
            })?;
            builder.add(glob);
        }
