...
pub trait Filesystem: Send + Sync {
    ...
    ///
    /// **Durability contract**: an `Ok` return means the bytes have reached the
    /// OS — the implementation MUST flush before returning. Callers rely on
    /// this to avoid losing events on crash.
...
///
/// The `append` operation opens the file with `create | append`, writes, and
/// **flushes** before returning `Ok`. This preserves the durability contract
/// established in Phase 2 (`FileAuditSink::record` must not return `Ok` until
/// the write syscall has completed).
...
impl Filesystem for RealFilesystem {
    ...
    async fn append(&self, path: &Path, bytes: &[u8]) -> anyhow::Result<()> {
        ...
        // Flush ensures the write syscall has completed before we return Ok.
        // Do NOT remove this — it is the durability guarantee callers depend on.
        file.flush().await?;
        Ok(())
    }
    ...
    async fn read_dir(&self, path: &Path) -> anyhow::Result<Vec<PathBuf>> {
        ...
        loop {
            match entries.next_entry().await {
                ...
                Ok(None) => break,
                // Intentionally skip an unreadable entry and continue the scan.
                Err(_) => continue,
            }
        }
        Ok(paths)
