...
use mcp_flowgate_core::audit::{MemoryAuditSink, NullAuditSink};
...
"#,
    ...
    assert_eq!(merged.pointer("/audit/sink").unwrap(), "stdout");
}
...
async fn capability_ref_emits_full_audit_trail() {
    ...
    let runtime = WorkflowRuntime::new(
        ...
        audit.clone() as Arc<dyn mcp_flowgate_core::audit::AuditSink>,
    );
    let started = runtime
        .start(StartWorkflow {
            definition_id: "demo".into(),
