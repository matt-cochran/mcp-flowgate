...
//!
//! The validator under test is `mcp_flowgate_core::validate::validate_workflows`
...
// ---------- Orchestrators don't trigger V3 ----------

#[test]
fn v3_does_not_fire_on_orchestrators() {
    // `flow.*` workflows are tier-flow; V3 is cap-only. V8 ("orchestrator
...
"#;
    ...
    assert!(
        !has_error_containing(&diags, "MISSING_SNIPPET"),
        "orchestrator should not trigger V3: {diags:?}"
    );
}
