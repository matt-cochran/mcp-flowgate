...
fn examples_dir() -> PathBuf {
    ...
    p.pop(); // crates/mcp-flowgate-core → crates
...
fn is_rejected(resp: &Value) -> bool {
    resp.pointer("/result/status").and_then(Value::as_str) == Some("rejected")
}
...
#[tokio::test]
#[ignore = "swe_agent requires LLM-driven output: planning state has `delegate: planning-agent` \
            and its `plan_ready` transition needs `normalizedProblem` + `risk` from the model. \
            The walker has no SubAgentSpawner — covered by Tranche-2 ETE smoke against a real \
            model. Run with: `cargo test --test walk_examples -- --ignored swe_agent`"]
async fn swe_agent_walks_to_critiquing() {
    let resolved = load_example("swe-agent.yaml");
    let runtime = build_runtime(&resolved);
    let (final_state, steps, _terminal) = walk_workflow(&runtime, "swe_agent", 20).await;
    assert_eq!(
        final_state, "critiquing",
        "expected critiquing, got {final_state} after {steps} steps"
    );
    assert!(steps >= 5, "expected at least 5 steps, got {steps}");
}
...
#[ignore = "parallel_all maps `summary.ok_count` / `summary.verdict` from the real ParallelExecutor's \
            ...
            covered by crates/mcp-flowgate-executors/tests/parallel_executor.rs (29 tests)."]
...
#[ignore = "dynamic_fanout maps `summary.ok_count` from the real ParallelExecutor's output. Same \
            ...
            dynamic-fanout walking is covered by crates/mcp-flowgate-executors/tests/parallel_executor.rs."]
...
async fn output_mapping_pattern_walks_all_workflows() {
    ...
    // crates/mcp-flowgate-executors/tests/parallel_executor.rs (parallel output).
...
#[ignore = "evidence_quorum maps `reports: ...` to an array via a template that resolves to a \
            ...
            mechanics are covered by crates/mcp-flowgate-core/tests/evidence_guard.rs."]
...
#[ignore = "recovery_demo maps `attempt_result` from an executor-specific field the noop registry \
            ...
            crates/mcp-flowgate-core/tests/reliability.rs and dedicated executor tests."]
...
#[tokio::test]
#[ignore = "Duplicate coverage of swe_agent_walks_to_critiquing — same LLM-input requirement \
            (planning state has delegate: planning-agent). Decision-point detection itself is \
            structurally verified in production walker code; see runtime_chain.rs."]
async fn tdd_walk_workflow_detects_decision_point() {
    let resolved = load_example("swe-agent.yaml");
    let runtime = build_runtime(&resolved);
    let (final_state, steps, terminal) = walk_workflow(&runtime, "swe_agent", 20).await;
    assert_eq!(final_state, "critiquing");
    assert!(!terminal, "decision point is not terminal");
    assert_eq!(steps, 5);
}
...
#[tokio::test]
async fn tdd_ai_review_swarm_workflow_config_resolves() {
    // Proves: the ai-review-swarm YAML resolves cleanly through
    // the config pipeline. This is the first gate — if this fails,
    // the workflow can't even be loaded.
    let _resolved = load_example("ai-review-swarm/gateway.yaml");
}

#[tokio::test]
async fn tdd_ai_review_swarm_has_seven_reviewer_branches() {
    // Proves: the review_swarm state's parallel executor declares
    // exactly 7 branches (one per reviewer role).
    let resolved = load_example("ai-review-swarm/gateway.yaml");
    let branches = resolved
        .pointer(
            "/workflows/ai_review_swarm/states/review_swarm/transitions/fan_out/executor/branches",
        )
        .and_then(Value::as_array)
        .expect("branches array");
    assert_eq!(branches.len(), 7, "must have exactly 7 reviewer branches");
}

#[tokio::test]
async fn tdd_ai_review_swarm_uses_continue_on_failure() {
    // Proves: on_branch_failure is `continue` — a single reviewer
    // failure must not cancel the other 6. If this were `bail`,
    // one flaky reviewer would kill the whole swarm.
    let resolved = load_example("ai-review-swarm/gateway.yaml");
    let on_failure = resolved
        .pointer("/workflows/ai_review_swarm/states/review_swarm/transitions/fan_out/executor/on_branch_failure")
        .and_then(Value::as_str);
    assert_eq!(
        on_failure,
        Some("continue"),
        "must continue on branch failure"
    );
}

#[tokio::test]
async fn tdd_ai_review_swarm_all_nine_skills_declared() {
    // Proves: all 9 skill fragments are declared (7 reviewers +
    // merge-dedupe + autofix). A missing skill would cause
    // config load to fail.
    let resolved = load_example("ai-review-swarm/gateway.yaml");
    let skills = resolved
        .pointer("/skills")
    ...
    for s in &required {
        assert!(skills.contains_key(*s), "missing skill: {s}");
    }
}

#[tokio::test]
async fn tdd_ai_review_swarm_human_arbitration_is_actor_human() {
    // Proves: the human_arbitration state declares `actor: human`.
    // Without this, an LLM could auto-accept findings — defeating
    // the human-in-the-loop invariant.
    let resolved = load_example("ai-review-swarm/gateway.yaml");
    let actor = resolved
        .pointer("/workflows/ai_review_swarm/states/human_arbitration/actor")
        .and_then(Value::as_str);
    assert_eq!(
        actor,
        Some("human"),
        "human_arbitration must be actor: human"
    );
}
