//! SPEC §23 drift detection — assert that the verb/root tables in SPEC.md
...
//!
//! Strategy: read SPEC.md at test time, slice it by section header, extract
...
fn read_spec() -> String {
    let path = workspace_root().join("SPEC.md");
    std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("SPEC.md must exist at {}: {e}", path.display()))
}
...
fn slice_section<'a>(src: &'a str, marker_prefix: &str) -> &'a str {
    let start = src
        .find(marker_prefix)
        .unwrap_or_else(|| panic!("section header '{marker_prefix}' not found in SPEC.md"));
...
fn spec_5_4_2_matches_blessed_subject_roots_bidirectionally() {
    ...
    // backticks for context: the `flowgate.strict_namespacing` flag
