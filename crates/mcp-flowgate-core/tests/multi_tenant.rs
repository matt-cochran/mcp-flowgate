...
use mcp_flowgate_core::audit::MemoryAuditSink;
...
fn build_runtime() -> WorkflowRuntime {
    ...
    let audit: Arc<mcp_flowgate_core::audit::MemoryAuditSink> = Arc::new(MemoryAuditSink::new());
    let definitions = Arc::new(ConfigDefinitionStore::from_config(&config));
    let store: Arc<dyn mcp_flowgate_core::ports::WorkflowStore> =
