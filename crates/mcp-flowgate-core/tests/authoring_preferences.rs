//! SPEC §17.x (v0.3) — `flowgate.authoring.*` advisory preferences.
//! Surfaced to LLM-driven authoring workflows via template substitution
//! (`{{$.flowgate.authoring.*}}`). FMECA atomic assertions for shape
...
fn all_workflows_share_the_same_preferences_snapshot() {
    // Per SPEC §8.4 the `flowgate.*` block is gateway-wide; per-workflow
...
async fn template_substitutes_preferred_script_language_into_skill_guidance() {
    ...
    impl ExecutorRegistry for NoopRegistry {
        fn get(&self, _: &str) -> Option<Arc<dyn mcp_flowgate_core::Executor>> {
            None
        }
    }
...
workflows:
  demo:
    ...
    states:
      writing:
        goal: "Write a new script in {{$.flowgate.authoring.preferred_script_language}}"
        guidance: "Default to {{$.flowgate.authoring.preferred_script_language}} unless a cross-platform need says otherwise."
        transitions:
          done:
            target: terminal
            executor: { kind: noop }
...
workflows:
  demo:
    ...
    states:
      writing:
        goal: "Lang: {{$.flowgate.authoring.preferred_script_language}}"
        transitions:
          done:
            target: terminal
            executor: { kind: noop }
