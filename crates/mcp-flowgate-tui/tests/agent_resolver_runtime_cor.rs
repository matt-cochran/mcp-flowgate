...
//!  - all-bindings-fail returns structured `AgentResolutionExhausted`
...
use mcp_flowgate_tui::agent_resolver::{
    AgentsFile, Binding, ConfigSource, Delegate, FailureClass, Provider, ProviderFeatures, Resolver,
};
...
fn legacy_registry_default_resolve_bindings_returns_single_entry() {
    ...
    use mcp_flowgate_tui::interpreter::LegacyAgentRegistry;
    ...
    assert!(matches!(list.bindings[0].provider, Provider::Anthropic));
...
#[allow(dead_code)]
fn _import_touch(_: &Binding, _: &ProviderFeatures, _: &Delegate) {}
