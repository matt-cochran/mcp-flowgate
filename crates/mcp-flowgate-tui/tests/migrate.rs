//! T21 — tests for the `flowgate migrate-agents-from-cli` migration.
//!
//! Two test surfaces:
//!  - `cli_args_to_yaml`: pure translation (the meat of the migration).
//!  - `write_atomic`: filesystem-touching helper. Verified via tempdir.

use mcp_flowgate_tui::agent_resolver::AgentsFile;
use mcp_flowgate_tui::migrate::{cli_args_to_yaml, write_atomic, MigrationError};
...
fn single_override_promotes_to_default_too() {
    ...
    let parsed = AgentsFile::from_yaml(&yaml).expect("round-trips");
    assert_eq!(parsed.default.len(), 1);
...
fn write_atomic_creates_parent_dir() {
    ...
    let target = tmp.path().join("nested/path/agents.yaml");
...
fn write_atomic_overwrites_existing() {
    let tmp = tempfile::tempdir().unwrap();
    let target = tmp.path().join("agents.yaml");
...
fn end_to_end_migrate_and_validate() {
    ...
    AgentsFile::from_path(&target).expect("on-disk file parses through the real loader");
}
