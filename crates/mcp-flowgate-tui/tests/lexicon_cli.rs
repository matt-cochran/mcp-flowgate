//! CLI tests for `flowgate lexicon` subcommand suite.
//!
//! Each test asserts ONE behavior.
//! Pattern: spawn the real `flowgate` binary via
//! `std::process::Command::new(env!("CARGO_BIN_EXE_flowgate"))`.
...
/// Write a minimal `flowgate.yaml` with:
/// - a defined term `evidence-pack`
/// - a script `build.evidence-foo` which creates a PENDING_DEFINITION
///   placeholder for `evidence-foo`
fn write_fixture_config(dir: &std::path::Path) -> std::path::PathBuf {
    let path = dir.join("flowgate.yaml");
    let content = r#"
version: "1.0.0"
flowgate:
...
fn list_exits_nonzero_without_config() {
    let out = Command::new(binary())
        .args(["lexicon", "list"])
        // Ensure FLOWGATE_CONFIG is not set in environment.
        .env_remove("FLOWGATE_CONFIG")
        .output()
