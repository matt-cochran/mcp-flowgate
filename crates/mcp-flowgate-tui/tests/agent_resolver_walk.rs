...
use mcp_flowgate_tui::agent_resolver::{
    AgentsFile, ConfigSource, Delegate, FailureClass, Resolver,
};
...
fn delegate_parses_affinity_tier() {
    let d = Delegate::parse("coding-frontier").expect("parses");
...
fn delegate_parses_affinity_only() {
    let d = Delegate::parse("coding").expect("parses");
    assert!(d.affinity.is_some());
...
fn delegate_parses_tier_only() {
    let d = Delegate::parse("frontier").expect("parses");
...
fn delegate_parses_hyphenated_affinity() {
    ...
    // `web` + `search`.
    let d = Delegate::parse("web-search").expect("parses");
    assert!(d.affinity.is_some());
    assert!(d.tier.is_none());
}
...
fn delegate_parses_hyphenated_affinity_with_tier() {
    let d = Delegate::parse("web-search-frontier").expect("parses");
    assert!(d.affinity.is_some());
    assert!(d.tier.is_some());
}
...
fn delegate_rejects_garbage() {
    Delegate::parse("nonsense").expect_err("garbage rejected");
    Delegate::parse("").expect_err("empty rejected");
}
...
"#;
    let r = resolver_from(yaml);
    let d = Delegate::parse("prose-commoditized").unwrap();
...
"#;
    let r = resolver_from(yaml);
    let d = Delegate::parse("recon-commoditized").unwrap();
