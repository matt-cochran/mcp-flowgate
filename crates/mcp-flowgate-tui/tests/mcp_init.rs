//! Integration tests for `flowgate mcp init`.
//!
//! The subcommand body lives in `crates/mcp-flowgate-tui/src/mcp_init.rs`.
...
use mcp_flowgate_tui::mcp_init::{run_init, McpInitArgs};
...
fn force_flag_overwrites_existing_file() {
    ...
    let body = std::fs::read_to_string(&p).unwrap();
    assert!(
        body.contains("mcpServers") && body.contains("flowgate"),
