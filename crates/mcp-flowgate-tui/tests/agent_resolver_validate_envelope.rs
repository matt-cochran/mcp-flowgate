//! PR2 — `validate_agents_config_envelope` tests.
//!
//! Backs the `flowgate validate-agents-config <path>` CLI subcommand
//! that the meta library's `cap.implement.write-agents-config` shells
//! out to for post-write round-trip validation (FMECA U3).
//!
...
use mcp_flowgate_tui::agent_resolver::validate_agents_config_envelope;
...
fn envelope_io_error_surfaces_named_kind_on_missing_path() {
    let env = validate_agents_config_envelope(std::path::Path::new(
        "/definitely/does/not/exist/agents.yaml",
    ));
    assert_eq!(env["ok"], serde_json::json!(false), "envelope: {env:#}");
    assert_eq!(
