...
//! requires `ANTHROPIC_API_KEY` (or equivalent) AND a running mcp-flowgate
...
#[tokio::test]
#[ignore = "requires ANTHROPIC_API_KEY and a live mcp-flowgate binary; run with: cargo test -- --ignored sub_agent_smoke"]
async fn live_spawn_against_real_aether_completes_or_times_out() {
    ...
    //   - mcp-flowgate binary on PATH or via MCP_FLOWGATE_PATH
