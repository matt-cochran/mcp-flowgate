...
use mcp_flowgate_tui::agent_resolver::{AgentsFile, Binding, Provider, ProviderFeatures};
use mcp_flowgate_tui::doctor_probe_cache::{
...
overrides:
  coding:
    - provider: { name: ollama }
      model: llama3
"#;
    let file = AgentsFile::from_yaml(yaml).expect("parses");
...
async fn local_providers_are_skipped() {
    ...
    for p in [Provider::Ollama, Provider::Lmstudio] {
        let b = Binding {
            provider: p.clone(),
            model: "any".into(),
