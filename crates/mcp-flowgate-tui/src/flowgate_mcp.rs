//! Sole MCP wiring — makes mcp-flowgate the only MCP server aether sees.
//!
//! This is the architectural invariant: the model has exactly one MCP tool
//! surface — Flowgate's 7 stable tools. Every other capability (filesystem,
//! shell, external APIs, existing aether tools) is available only through
//! Flowgate's `connections:` proxy layer.
//!
...
//!   ▼
//! mcp-flowgate (child process, sole MCP server)
...
//!
//! Flowgate replaces aether's tool surface entirely. There is no bypass path:
...
/// Make mcp-flowgate the **sole** MCP server.
///
/// Replaces whatever MCP config aether would normally use with a single
/// entry pointing at the mcp-flowgate child process. The gateway YAML
/// (loaded by mcp-flowgate) defines what downstream tools are reachable
/// through its executor proxy layer.
///
/// This is intentionally aggressive: clear + replace, not append.
///
/// Fails fast (B.3 in the audit-resolution plan) when the mcp-flowgate
/// binary cannot be located AND `MCP_FLOWGATE_PATH` is explicitly set but
...
/// Generate the MCP config JSON that wires aether to mcp-flowgate as its
/// sole MCP server.
fn sole_mcp_config() -> Result<String> {
    let binary = find_flowgate_binary()?;
    Ok(serde_json::json!({
        "mcpServers": {
            "flowgate": {
                "command": binary,
                "env": {
                    "FLOWGATE_CONFIG": default_config_path(),
                }
            }
        }
    })
...
/// Locate the `mcp-flowgate` binary. Resolution order:
/// 1. `MCP_FLOWGATE_PATH` env var if set. **Errors fast** with
///    `MCP_FLOWGATE_NOT_FOUND` if the path doesn't exist — operator-supplied
///    paths are an explicit contract, never silently fall back.
/// 2. Sibling next to `current_exe()` (bundled deployment).
/// 3. Bare string `"mcp-flowgate"` (relies on PATH; spawn errors will be
///    actionable on their own).
///
/// Pub(crate) so the binary_discovery test suite can exercise it directly.
pub(crate) fn find_flowgate_binary() -> Result<String> {
    // 1. Operator override via env var — strict.
    if let Ok(override_path) = std::env::var("MCP_FLOWGATE_PATH") {
        if !override_path.trim().is_empty() {
            let p = std::path::Path::new(&override_path);
            if !p.exists() {
                return Err(anyhow!(
                    "MCP_FLOWGATE_NOT_FOUND: MCP_FLOWGATE_PATH is set to \
                     '{override_path}' but no file exists at that path. \
                     Unset the env var to fall back to discovery, or install \
                     mcp-flowgate (e.g. `cargo install mcp-flowgate`)."
                ));
            }
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let sibling = dir.join("mcp-flowgate");
            if sibling.exists() {
                return Ok(sibling.to_string_lossy().to_string());
            }
            let sibling_exe = dir.join("mcp-flowgate.exe");
    ...
    tracing::debug!(
        "mcp-flowgate not found as sibling of current_exe(); falling back to \
         PATH lookup. If spawn fails, set MCP_FLOWGATE_PATH or `cargo install mcp-flowgate`."
    );
    Ok("mcp-flowgate".to_string())
}

/// Default config path. When not overridden by FLOWGATE_CONFIG env var,
/// mcp-flowgate falls back to looking for `flowgate.yaml` in the current
/// working directory.
