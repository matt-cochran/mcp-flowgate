//! `flowgate mcp init` — generate MCP client config files for the project.
//!
//! ACP-mode sessions (`flowgate acp`) and most MCP-host editors (Cursor,
...
//! them accept the MCP config via CLI args the way `flowgate headless`
//! does, so the sole-MCP wiring has to happen via file generation rather
//! than programmatic injection.
//!
//! This module is the file-generator. `flowgate mcp init` writes the
//! flowgate MCP server entry to each requested config file, using the
//! same `find_flowgate_binary` resolution rules as the headless path.
//!
...
/// Entry point. Resolves the target directory, builds the flowgate
...
pub fn run_init(args: &McpInitArgs) -> Result<()> {
    ...
    let config_json = build_flowgate_mcp_config_json(&dir)?;
    ...
    if args.claude_desktop {
        ...
        println!(
            "  note: Claude Desktop's actual config lives at:\n\
             ...
             merge the `mcpServers.flowgate` entry from the generated file \
...
/// Build the MCP server config JSON that wires flowgate as the sole
/// MCP server. Same shape as `flowgate_mcp::sole_mcp_config()` so an
/// operator generating the file gets the same wiring `run_headless`
/// would inject programmatically.
///
/// The `dir` argument seeds `FLOWGATE_CONFIG` — if there's a
/// `flowgate.yaml` in that directory, we point at it; otherwise we
/// leave the env unset and let mcp-flowgate's own resolution fall
/// back to cwd at spawn time.
fn build_flowgate_mcp_config_json(dir: &Path) -> Result<String> {
    let binary = flowgate_mcp::find_flowgate_binary()?;
    let gateway_yaml = dir.join("flowgate.yaml");

    let env = if gateway_yaml.exists() {
        json!({ "FLOWGATE_CONFIG": gateway_yaml.to_string_lossy() })
    } else {
        json!({})
    };
