//! SPEC §30.10.7 — `flowgate lexicon` CLI subcommand suite.
//!
//! Provides operator-facing out-of-band management of the lexicon:
//!
//! ```text
//! flowgate lexicon define <term> --definition-short "..." [options]
//! flowgate lexicon alias <term> --add <alias>
//! flowgate lexicon cancel <term>
//! flowgate lexicon list [--bounded-context X]
//! flowgate lexicon pending
//! ```
//!
//! ## Storage note
//!
//! The current lexicon backend is an in-memory overlay over the config-file
//! lexicon block (`flowgate.yaml`). The `define`, `alias`, and `cancel`
...
//! `lexicon:` block in `flowgate.yaml` directly and reload.
//!
//! The `list` and `pending` subcommands read from the _resolved_ config file
//! (loaded via `--config` or `$FLOWGATE_CONFIG`). They reflect the
...
/// `flowgate lexicon <subcommand>` — out-of-band lexicon management.
#[derive(Subcommand, Debug)]
pub enum LexiconCmd {
    /// Define or redefine a lexicon term.
    ///
    /// Creates (or overwrites) a term in the in-memory overlay. To persist,
    /// edit the `lexicon:` block in `flowgate.yaml` and reload.
    Define(DefineArgs),
    ...
    ///
    /// Reads from `--config` / `$FLOWGATE_CONFIG` and emits one JSON object
...
pub struct DefineArgs {
    ...
    /// Path to `flowgate.yaml` (config). When omitted, falls back to
    /// `$FLOWGATE_CONFIG`. Required to populate the overlay from the
...
pub struct AliasArgs {
    ...
    /// Path to `flowgate.yaml` for overlay initialisation.
    #[arg(long, env = "FLOWGATE_CONFIG")]
    pub config: Option<String>,
}
...
/// Dispatch `flowgate lexicon <subcommand>`.
...
fn run_define(args: DefineArgs) -> Result<ExitCode> {
    ...
    let mut entry = mcp_flowgate_core::lexicon::build_entry(
    ...
    let result = json!({
        "term": args.term,
        "entry": entry,
        "persisted_to": "overlay (in-memory only; edit flowgate.yaml to persist)"
    });
    println!("{result}");
    Ok(ExitCode::SUCCESS)
}
...
/// Load and resolve the config file at the given path (or `$FLOWGATE_CONFIG`).
...
fn require_config(config_arg: Option<&str>, subcommand: &str) -> Result<Value> {
    ...
    match path {
        None => anyhow::bail!(
            "lexicon {subcommand}: no config path. Pass --config or set $FLOWGATE_CONFIG."
        ),
        Some(p) => {
            ...
            mcp_flowgate_core::config::resolve(value).map_err(|e| anyhow::anyhow!("{e}"))
        }
    }
}
...
/// by `mcp_flowgate_core::lexicon::*` helpers: `{ "_lexiconLibrary": { ... } }`.
