...
use mcp_flowgate_core::reliability::{execute_with_reliability, ReliabilityPolicy};
...
impl PipelineExecutor {
    ...
    pub fn set_registry(&self, registry: Arc<dyn ExecutorRegistry>) {
        let _ = self.executors.set(registry);
    }
...
async fn run_steps(
    ...
    for (index, step_cfg) in cfg.steps.iter().enumerate() {
        ...
        let policy = ReliabilityPolicy::from_value(step_cfg.get("reliability"));
