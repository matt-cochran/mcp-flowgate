...
//! Flowgate fragment shape. v1 handles mattpocock-style `.claude/skills/*.md`
...
use mcp_flowgate_core::discovery::Verb;
...
fn split_frontmatter(raw: &str) -> (serde_json::Map<String, Value>, String) {
    let stripped = raw.trim_start();
    if !stripped.starts_with("---") {
        return (serde_json::Map::new(), raw.to_string());
    }
    ...
    let parsed: Value = serde_yaml::from_str(fm_text).unwrap_or(Value::Null);
    let map = parsed
        .as_object()
        .cloned()
