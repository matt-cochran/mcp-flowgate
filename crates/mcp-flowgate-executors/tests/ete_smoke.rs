...
use mcp_flowgate_core::config::load_resolved;
...
fn examples_dir() -> PathBuf {
    ...
    p.pop(); // crates/mcp-flowgate-tui → crates
...
async fn smoke_ete_walks_to_ship_via_v04_primitives() {
    ...
    let mut resp = start;
    while steps < 20 {
        if resp.pointer("/result/status").and_then(Value::as_str) == Some("completed") {
            break;
        }
...
async fn smoke_ete_path_allowlist_rejects_disallowed_path() {
    ...
    use mcp_flowgate_core::slot_constraint::evaluate_constraints;
    let path = examples_dir().join("smoke-ete/gateway.yaml");
