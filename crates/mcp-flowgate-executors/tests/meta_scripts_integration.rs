//! PR3 — exercise the three flowgate-meta production scripts via the
//! real ScriptExecutor (not the fixture short-circuit). The meta
//! orchestrators e2e bypasses these via CapShortCircuit; this file
...
//!
//! Wiring note: `CARGO_BIN_EXE_flowgate` only resolves in the crate that
//! *owns* the binary. Since the `flowgate` binary belongs to
//! `mcp-flowgate-tui` (a separate crate), we locate it at runtime via
//! `flowgate_bin_path()`, which navigates up to the workspace root and
//! finds `target/debug/flowgate` (or `target/release/flowgate` if present).
//! Build the binary before running this test file in isolation:
//!   cargo build -p mcp-flowgate-tui --bin flowgate
...
use mcp_flowgate_executors::ScriptExecutor;
use serde_json::{json, Value};

/// Locate the `flowgate` binary in the Cargo target directory.
/// `CARGO_BIN_EXE_flowgate` only works in the owning crate (mcp-flowgate-tui);
/// from a separate crate we navigate to the workspace root and look under
/// target/{profile}/flowgate. The CARGO_BUILD_TARGET_DIR / OUT_DIR env vars
/// aren't set at test-run time, so we use the conventional `target/` sibling
/// of the workspace Cargo.toml.
fn flowgate_bin_path() -> String {
    // env!("CARGO_MANIFEST_DIR") is .../crates/mcp-flowgate-executors at compile time.
    // Pop up two levels to get the workspace root.
    ...
    if release.exists() {
        ...
        assert!(
            debug.exists(),
            "flowgate binary not found at {} or {}. Run `cargo build -p mcp-flowgate-tui` first.",
            release.display(),
            debug.display()
        );
...
/// Load the script body from the fixture copy of the flowgate-meta
...
fn script_body(subject: &str) -> String {
    ...
    path.pop(); // mcp-flowgate-executors
    path.pop(); // crates
    path.push("crates");
    path.push("mcp-flowgate-core");
    path.push("tests");
    path.push("fixtures");
    path.push("flowgate-meta");
    path.push("scripts-library");
...
async fn install_agents_config_writes_atomically_and_rolls_back_on_invalid() {
    ...
    let target = tmpdir.path().join("agents.yaml");
    let flowgate_bin = flowgate_bin_path();
...
async fn auth_only_smoke_test_classifies_per_binding() {
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let agents = tmpdir.path().join("agents.yaml");
    std::fs::write(
        &agents,
        "version: 1\ndefault:\n  - provider:\n      name: anthropic\n    model: claude-sonnet-4-6\n  - provider:\n      name: openai\n    model: gpt-5\n",
    )
