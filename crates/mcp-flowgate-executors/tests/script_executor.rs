...
// ── FLOWGATE_SCRIPT_SUBJECT + FLOWGATE_SCRIPT_HASH exposed to script ────

#[tokio::test]
async fn flowgate_env_vars_exposed_to_script_body() {
    let hash = "sha256:deadbeef00000000000000000000000000000000000000000000000000000000";
    let instance = instance_with_scripts_library(json!({
        "verify.env.exposed": {
            "verb": "verify",
            "lifecycle": "stable",
            "body": "echo \"sub=$FLOWGATE_SCRIPT_SUBJECT hash=$FLOWGATE_SCRIPT_HASH\"",
            "hash": hash,
            "source": "config"
        }
    }));
