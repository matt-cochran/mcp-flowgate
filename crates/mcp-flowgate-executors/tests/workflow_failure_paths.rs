...
//!   (no auto-advance to target), result.status is NOT "completed".
...
// silently report status="completed". The chain runner captures the failure
...
async fn permanent_executor_failure_does_not_report_completed() {
    ...
    // The contract: permanent executor failure must NOT silently say "completed".
    // Expected: status="failed", state="ready" (originating state, non-terminal,
    // so the response() method cannot upgrade it to "completed").
    assert_ne!(
        status, "completed",
...
async fn guard_failure_on_submit_blocks_advance() {
    ...
    assert_eq!(
        ...
        "guard failure should report status='rejected'; \
