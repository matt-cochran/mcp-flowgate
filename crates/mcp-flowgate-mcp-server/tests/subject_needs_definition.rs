...
//!
//! When `flowgate.command` is called with `definitionId` for a workflow whose
...
/// Config with a workflow that references an unresolved subject (no lexicon entry).
/// Uses a scripts block with `build.evidence-foo` so `evidence-foo` becomes a
/// pending placeholder in the workflow's `_lexiconLibrary`.
...
#[tokio::test]
async fn flowgate_command_start_with_unresolved_subject_returns_interaction() {
    ...
    assert_eq!(
        resp["queued_command"]["method"], "flowgate.command",
        "queued_command.method must be flowgate.command"
    );
    assert_eq!(
...
async fn link_as_alias_has_correct_shape() {
    ...
    assert_eq!(alias_link["method"], "flowgate.command");
...
async fn define_new_link_has_correct_shape() {
    ...
    assert_eq!(define_link["method"], "flowgate.command");
    assert!(
...
async fn cancel_link_has_correct_shape() {
    ...
    assert_eq!(cancel_link["method"], "flowgate.command");
    assert_eq!(
...
#[tokio::test]
async fn flowgate_command_start_with_resolved_subjects_proceeds_normally() {
