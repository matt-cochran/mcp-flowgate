...
async fn start_workflow(server: &FlowgateServer, definition_id: &str) -> String {
    let resp = server
        .dispatch_call(call(
            TOOL_COMMAND,
...
/// The `lookup_link` for an oversize entry uses `flowgate.query` as the method.
#[tokio::test]
async fn oversize_lookup_link_uses_flowgate_query_method() {
    let (server, _) = build_server(config_with_oversize_lexicon_entry());
    let wf_id = start_workflow(&server, "big_wf").await;
    let resp = server
        .dispatch_call(call(
    ...
    assert_eq!(
        method,
        Some("flowgate.query"),
        "lookup_link method must be flowgate.query; got: {resp}"
    );
}
