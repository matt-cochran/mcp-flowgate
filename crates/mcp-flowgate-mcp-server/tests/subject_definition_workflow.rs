...
//!      crates/mcp-flowgate-core/tests/lexicon.rs. This file
...
/// Config with a skills block referencing an unregistered subject.
...
/// Config with both a real entry AND a pending placeholder (two different subjects).
...
/// Every resolution link points at `flowgate.command` as its method.
#[tokio::test]
async fn subject_needs_definition_link_args_use_flowgate_command_method() {
    let (server, _) = build_server(config_with_pending_script_subject());
    let resp = server
        .dispatch_call(call(TOOL_COMMAND, json!({ "definitionId": "pending_wf" })))
        .await
    ...
    for link in links {
        assert_eq!(
            link["method"], "flowgate.command",
            "all resolution links must use flowgate.command method; got: {link}"
        );
    }
}
