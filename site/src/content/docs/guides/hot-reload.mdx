...
- **In-flight workflows** -- any workflow instance that's already started continues on its current definition. It doesn't get yanked mid-step. The model can still call `flowgate.query({workflowId})` (get) and `flowgate.command({workflowId, transition})` (submit) on existing instances.
...
3. Call `begin_drain` on the old instance (or let it drain naturally). It stops accepting new `flowgate.command({definitionId})` (start) calls but lets in-flight workflows finish.
