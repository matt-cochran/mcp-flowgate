...
The model gets the subjects (the menu), not the bodies (the meal). When it decides a particular skill is relevant, it calls `flowgate.query({subject})` to fetch the body. Progressive disclosure — the model pulls one mental model at a time, only when it's about to use it.
...
The [two MCP tools](/reference/tools/) are how the model discovers and reads skills:

### `flowgate.query` with `query` searches them
...
When `flowgate.command({definitionId})` creates an instance, the runtime stamps a `_skillsLibrary` onto the instance's definition snapshot — a copy of every skill referenced anywhere in the workflow, with body, hash, and verb included. The instance carries this for its entire lifetime. Subsequent `flowgate.query({workflowId})` (get) and `flowgate.command({workflowId, transition})` (submit) calls read from the stamp; they never re-query the live `skills:` config.
