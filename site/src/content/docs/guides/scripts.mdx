...
Same SPEC §8.2 invariant as `_skillsLibrary`. When `workflow.start` creates an instance, the runtime stamps a `_scriptsLibrary` onto the instance's definition snapshot — copy of every script the workflow references, with body, hash, and verb included.
