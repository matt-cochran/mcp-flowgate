...
All stores enforce optimistic locking. When you call `flowgate.command({workflowId, transition, expectedVersion})` (submit), you pass an `expectedVersion`. The store checks this against the current version before saving:
