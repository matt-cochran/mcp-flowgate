...
Closed enums let `gateway.scripts.search verb=build` filter cleanly without synonym matching. They also force authors to pick — `compile.cargo.release` versus `build.cargo.release` is a debate; with the closed enum there's no debate, it's `build`. Less bike-shedding, more shipping.
