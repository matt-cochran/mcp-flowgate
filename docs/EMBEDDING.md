# Embedding mcp-flowgate

A guide for **library users** — engineers building their own
HATEOAS-inspired MCP APIs on top of mcp-flowgate. If the README explains
...
nest gateways, etc. — see [`MCP-CONTROL-ARCHITECTURE.md`](./MCP-CONTROL-ARCHITECTURE.md).
...
- **You want a custom MCP tool surface.** The seven-tool stable surface
...
If none of those apply, just use `mcp-flowgate serve --config X.yaml` and
move on.
...
```
mcp-flowgate-core         runtime, ports, audit, reliability, capability,
                          discovery, config preprocessor, evidence,
                          in-memory + file + sqlite stores
mcp-flowgate-executors    cli / mcp / rest / human / noop executors,
                          tools/list importer
mcp-flowgate-mcp-server   the default seven-tool ServerHandler
                          (FlowgateServer)
mcp-flowgate-schema       typify-generated Rust types from JSON schemas
                          (optional convenience for callers)
```

You'll typically depend on `mcp-flowgate-core` plus
`mcp-flowgate-executors`. Add `mcp-flowgate-mcp-server` only if you want
the default seven-tool surface; skip it if you're rolling your own.

```toml
[dependencies]
mcp-flowgate-core      = "0.1"
mcp-flowgate-executors = "0.1"
mcp-flowgate-mcp-server = "0.1"   # optional: only for FlowgateServer
rmcp                    = "1.6"   # to serve over MCP
...
expose the standard seven-tool surface over stdio:
...
use mcp_flowgate_executors::default_registry;
...
async fn main() -> anyhow::Result<()> {
    ...
    let server = FlowgateServer::new(runtime).with_discovery(discovery);
...
mcp-flowgate is designed to support **compile-time** config too: bake
...
```rust
use mcp_flowgate_core::config::{deep_merge, resolve};
...
The default `FlowgateServer` exposes seven stable tools
(`gateway.{home,search,describe}` + `workflow.{start,get,submit,explain}`).
...
### Pattern: replace the seven-tool surface with your own
...
- **You give up the seven-tool stability invariant** because your tool
  list is your own. That's fine — invariants are about clients of the
  default `FlowgateServer`, not about the runtime itself.
...
Each cross-cutting concern is a trait in `mcp_flowgate_core::ports`.
...
```rust
use std::sync::Arc;
use mcp_flowgate_executors::HashMapExecutorRegistry;
...
let registry = HashMapExecutorRegistry::new()
    ...
    .with("noop",    Arc::new(mcp_flowgate_executors::NoopExecutor));
...
use mcp_flowgate_core::model::{Principal, WorkflowInstance};
use mcp_flowgate_core::ports::GuardEvaluator;
use serde_json::Value;
...
Combine multiple sinks if you want both stdout AND your sink:
...
Implement `mcp_flowgate_core::ports::WorkflowStore` to back instance
state with Postgres, Redis, etc. The trait has three methods:
...
fn main() {
    ...
    // Requires depending on mcp-flowgate-core in [build-dependencies].
}
```

For deeper validation, add `mcp-flowgate-core` to `[build-dependencies]`
and call `mcp_flowgate_core::config::resolve_str(&yaml)` in `build.rs`.
...
### 8a. Hybrid: keep FlowgateServer's seven tools, add custom ones

You don't have to choose. Wrap `FlowgateServer` in your own
`ServerHandler` that delegates the standard seven tools and adds your
own:

```rust
pub struct HybridServer {
    inner: FlowgateServer,
    /* domain state */
}

impl ServerHandler for HybridServer {
    async fn list_tools(&self, /*…*/) -> Result<ListToolsResult, McpError> {
        let mut tools = mcp_flowgate_mcp_server::tool_definitions();
    ...
    async fn call_tool(&self, request: CallToolRequestParams, ctx: RequestContext<RoleServer>)
        -> Result<CallToolResult, McpError> {
        if mcp_flowgate_mcp_server::STABLE_TOOL_NAMES.contains(&request.name.as_ref()) {
...
fn principal_from_request(ctx: &RequestContext<RoleServer>) -> Principal {
    ...
    //    Principal {
    //        subject: std::env::var("FLOWGATE_PRINCIPAL").unwrap_or_default(),
    //        roles: env_csv("FLOWGATE_ROLES"),
    //        permissions: env_csv("FLOWGATE_PERMISSIONS"),
    //    }
    Principal::anonymous()
}
```
...
async fn deploy_workflow_blocks_without_approval() {
    let config = mcp_flowgate_core::config::resolve_str(include_str!("../my.yaml"))?;
...
> Use `FlowgateServer` for the standard surface; replace it when you need
