...
How mcp-flowgate gets published and where it gets listed. This file is
...
mcp-flowgate is **not yet published to any package registry**. No `v*`
...
|-------------|-------|
| Name        | `mcp-flowgate` |
| Registry name | `io.github.matt-cochran/mcp-flowgate` |
| Tagline     | MCP gateway that fronts any number of tools, CLIs and APIs behind seven governed, audited tools. |
| Repository  | https://github.com/matt-cochran/mcp-flowgate |
| Language    | Rust |
| License     | Apache-2.0 |
...
`mcp-flowgate` → Package settings → set visibility to **Public**. The
...
```bash
curl "https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.matt-cochran/mcp-flowgate"
```

Notes:
- The registry only stores **metadata** — it points at the GHCR image,
  it does not host it.
- Namespace `io.github.matt-cochran/*` is granted by GitHub auth because
...
registry and from GitHub. Once step 2 is done they pick up mcp-flowgate
...
```markdown
- [matt-cochran/mcp-flowgate](https://github.com/matt-cochran/mcp-flowgate) 🦀 🏠 - MCP gateway that fronts any number of tools, CLIs and APIs behind seven governed, audited tools.
```
...
```
Add mcp-flowgate to Aggregators

mcp-flowgate is an MCP gateway: it fronts any number of MCP servers,
...
- **URL:** `https://github.com/matt-cochran/mcp-flowgate`
...
`https://github.com/matt-cochran/mcp-flowgate`.
...
form, which is enough for Smithery to list mcp-flowgate and install it
...
and listen on `$PORT`. mcp-flowgate serves over stdio only today, so
