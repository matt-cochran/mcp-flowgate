...
If the README answers "what is mcp-flowgate?", this doc answers "how do I
**design** with it?".
...
```
        ...
        │                       this gateway                       │
        │  ───────────────────────────────────────────────────     │
        │  exposes 7 stable MCP tools (gateway.* + workflow.*)     │
        │                                                          │
...
Calling `gateway.search` or `workflow.start("proxy_default")` finds these.
...
top-level `workflow.start({ definitionId: "with_artifact_lock" })` from a
...
2. Outer gateway has a `kind: mcp` connection pointing at the inner
   gateway's URL.
3. Outer gateway's executor calls `workflow.submit` against the inner
   gateway's stable tool surface — the inner workflow runs to completion
   and the outer gateway sees the final result.

This is "nested workflows" without any new abstraction: each gateway
exposes the seven stable tools — three for discovery, four for workflow
action — so any gateway can drive any other gateway's workflows.
...
1. Calls `workflow.start` internally with the given `definitionId` and
   `input` (path expressions resolved against the parent's context and
   arguments).
2. Polls `workflow.get` until the sub-workflow reaches a terminal state.
...
what makes the same `github.create_pr` useful from `workflow.start` (proxy
mode) and from a five-state governance workflow.
...
`gateway.search` lexically scores against title, id, tags, description, and
...
4. **What's the upward surface?** (Always: the seven stable tools.)
...
layer below already enforces everything below it. The seven-tool stable
surface means upstream layers don't care how deep the stack is — calling
`workflow.submit` to the team gateway hits the team gateway, which uses
...
header). See
[EMBEDDING.md §8d](EMBEDDING.md#8d-identity-wiring-principal-into-a-custom-server-surface)
...
### Rebuilding what `gateway.search` already does

Don't add tools to enumerate or describe capabilities. The seven-tool
surface is stable on purpose. If a model needs to learn about your
capabilities, that's `gateway.search` and `gateway.describe`. If the model
...
A model calls `gateway.search "create pr"`, finds it, calls
`workflow.start("proxy_default")`, then `workflow.submit({transition: github.create_pr, …})`.
Done.
