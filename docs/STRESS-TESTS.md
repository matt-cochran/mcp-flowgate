...
`crates/mcp-flowgate-core/tests/stress_scenarios.rs`.
...
Tested in `crates/mcp-flowgate-core/tests/deterministic_chain.rs`:
...
| DC-07 | Chain auto-executes after `workflow.submit`                             | Passed                                        |
...
| DC-14 | `workflow.explain` includes actor type and deterministic flag            | Passed                                        |
...
fields. Full guide: [LLM-GUIDANCE.md](LLM-GUIDANCE.md).
...
`crates/mcp-flowgate-executors/tests/workflow_executor.rs`.
...
1. Write the test in
   `crates/mcp-flowgate-core/tests/stress_scenarios.rs` as **the
   declarative form you wish worked**, even if it doesn't yet.
...
5. If it's a doc-and-pattern gap, add the recipe to
   [MCP-CONTROL-ARCHITECTURE.md](MCP-CONTROL-ARCHITECTURE.md) instead of
   growing the runtime.
