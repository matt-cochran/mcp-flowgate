...
If you haven't read [CONCEPTS.md](CONCEPTS.md) yet, start there.
...
> [MCP-CONTROL-ARCHITECTURE.md](MCP-CONTROL-ARCHITECTURE.md#identity-in-multi-tenant-deployments).

> **Designing for reuse.** When the same guard pattern repeats across
> transitions, hoist into a named capability and wrap it. See
> [MCP-CONTROL-ARCHITECTURE.md](MCP-CONTROL-ARCHITECTURE.md).
...
and `body`. See [LLM-GUIDANCE.md](LLM-GUIDANCE.md) for design patterns.
...
The model still only sees `workflow.{start,get,submit,explain}` — it
navigates the entire flow through the links each response returns
(HATEOAS-inspired; see [CONCEPTS.md](CONCEPTS.md)), and the engine
...
[CONFIG.md](CONFIG.md#deterministic-chaining) for the full reference.
...
Phase guidance is indexed by `gateway.search`, so queries can match
against goal and guidance text. See
[CONFIG.md](CONFIG.md#phase-guidance) for the YAML shape.
...
| `transition.requested`         | A `workflow.submit` call enters the runtime.                      |
...
Stdout / file write one JSON line per event. Memory is for tests. None
...
`correlationId` ties together every event from one `workflow.start` or
one `workflow.submit` so you can reconstruct the full causal chain.
...
AUDIT_PATH="/var/log/flowgate-audit.jsonl"
...
tail -F "$AUDIT_PATH" | while read -r line; do
  ...
  curl -s -X POST "$SLACK_WEBHOOK" \
    ...
    -d "{
      \"text\": \"*Approval needed*\\nQueue: \`$queue\`\\nTransition: \`$transition\`\\nWorkflow: \`$workflow\`\\nID: \`$id\`\\nResolve with: \`mcp-flowgate approvals resolve $id\`\"
    }"
done
```
    ...
    for line in f:
        ...
        if event.get("event_type") == "human.approval.requested":
            ...
            resp = create_linear_issue(
                ...
                f"**Workflow:** `{workflow}`\\n**Queue:** `{queue}`\\n**Transition:** `{transition}`\\n**Event ID:** `{eid}`\\n\\nResolve with: `mcp-flowgate approvals resolve {eid}`"
            )
...
The built-in `mcp-flowgate approvals tail` command provides a
terminal-based approval watcher:

```bash
# Start the gateway with file audit
mcp-flowgate serve --config gateway.yaml &

# In another terminal, tail approvals
mcp-flowgate approvals tail --config gateway.yaml
```
...
```bash
mcp-flowgate approvals list --config gateway.yaml
```

To resolve an approval:

```bash
mcp-flowgate approvals resolve evt_a1b2c3 --outcome approved
mcp-flowgate approvals resolve evt_d4e5f6 --outcome rejected
```

Combine with `watch` for a refresh loop:

```bash
watch -n 5 'mcp-flowgate approvals list --config gateway.yaml'
```
...
and the CLI tail for ad-hoc debugging. The `mcp-flowgate approvals`
subcommand is the common resolution path across all three.

## Where to next

- The full configuration reference: [CONFIG.md](CONFIG.md)
- Composing this for larger systems: [MCP-CONTROL-ARCHITECTURE.md](MCP-CONTROL-ARCHITECTURE.md)
