...
4. Construct `workflow.submit({ workflowId, expectedVersion, transition, arguments: <merged> })`.
...
2. **Add an `expr` guard** that checks the field equals the
   ...
   `expr: "$.arguments.base == $.workflow.input.base"`. (Currently
   numeric comparisons only — string equality is a future addition.)
...
Both fields are indexed by `gateway.search`, so they improve
discoverability as well as runtime guidance.
