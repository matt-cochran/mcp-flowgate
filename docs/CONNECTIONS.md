...
query/body. See [CONFIG.md](CONFIG.md#executor-kinds) for the executor
shape.
...
`gateway.search` and `workflow.submit` can use it just like a declared
exposure.
...
See [MCP-CONTROL-ARCHITECTURE.md](MCP-CONTROL-ARCHITECTURE.md) for the
...
- Full schema reference for connections + executors: [CONFIG.md](CONFIG.md)
...
- Reliability and retry semantics for executor calls:
  [GOVERNANCE.md](GOVERNANCE.md#reliability-timeout--retry--fallback)
