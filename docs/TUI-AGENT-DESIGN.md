...
Add a `crates/mcp-flowgate-tui/` crate that wraps the Aether agent
framework (TUI, model calling, session management), routes ALL model
tool calls through the existing `mcp-flowgate` MCP server as its sole
tool surface, and adds a **deterministic graph-walking interpreter**
that eliminates orchestrator-LLM context accumulation by spawning
...
|---|---|---|
| `delegate` on Flowgate states (pass-through string) | Essential | **Yes** |
...
### Phase 1: Flowgate schema additions (pass-through, no logic)

Flowgate stays model-agnostic. It carries one new field that the TUI
...
**File:** `crates/mcp-flowgate-core/src/runtime_response.rs`
...
**File:** `crates/mcp-flowgate-core/src/config.rs`
...
**File:** `crates/mcp-flowgate-core/src/validate.rs`

Warn when a `delegate` references a name not in any known registry.
Soft check — Flowgate can't verify agent configs exist in the TUI.
...
and the 7 Flowgate tools.

### Phase 2: TUI interpreter — `crates/mcp-flowgate-tui/`

The crate already exists as `flowgate-agent` with Aether deps and
Flowgate MCP wiring. Phase 2 adds the interpreter.
...
async fn walk_workflow(runtime: &mut TuiRuntime, workflow_id: &str) -> Result<Value> {
    let mut retries = 0;
    loop {
        let resp = runtime.mcp.call("workflow.get", json!({"workflowId": workflow_id})).await?;
        ...
        if actionable.len() == 1 {
            runtime.mcp.call("workflow.submit", actionable[0]["args"].clone()).await?;
            retries = 0;
            continue;
        }
        ...
        runtime.mcp.call("workflow.submit", first["args"].clone()).await?;
        retries = 0;
        continue;
    }
}
...
/// same 7 Flowgate tools. Runs until it calls workflow.submit or
/// hits timeout/step limit.
async fn spawn_and_wait(&mut self, agent_name: &str, flowgate_resp: &Value) -> Result<()> {
    ...
    let session = aether_cli::session::Session::new()
        ...
        .with_tools(self.mcp_tools.clone())  // same 7 Flowgate tools
...
TOML files (`~/.flowgate/agents/*.toml`) are a v2 UX improvement.
...
- **flowgate-agent TUI binary** (`crates/mcp-flowgate-tui/`). Wraps
  the Aether agent framework with mcp-flowgate as its sole MCP server.
  ...
  orchestrator-LLM context accumulation.
...
Install `flowgate-agent` (or `cargo install mcp-flowgate --bin flowgate-agent`):
...
Add `crates/mcp-flowgate-tui/` to the workspace layout section.
...
1. **Flowgate schema + runtime** (Phase 1 — ~1.5 hours)
...
2. **TUI interpreter** (Phase 2 — ~5 hours)
   ...
   - Connect Aether's session API to Flowgate MCP
...
- Flowgate's 7-tool surface (stable)
- Flowgate's non-goal of "no LLM calling" (unchanged — the TUI calls
  models, Flowgate governs)
...
| F2 | Runtime | Sub-agent submits to wrong transition | Medium | Medium | Flowgate rejects `INVALID_TRANSITION`; sub-agent retries | Low | Low |
...
| F5 | Runtime | Flowgate process crashes | High | Low | WorkflowStore persistence; TUI reconnects + resume from snapshot | Medium | Low |
| F6 | Runtime | Sub-agent output fails inputSchema | Medium | Medium | Flowgate rejects before advancing; sub-agent retries or times out | Low | Low |
...
| F10 | Architecture | Interpreter advances to wrong state | High | Low | Interpreter never computes state — reads from Flowget.get | Low | Low |
| F11 | Delivery | Aether API breaks | Medium | Low | Pinned Cargo deps; CI catches | Low | Low |
| F12 | Runtime | Stale workflow.get response | Medium | Low | Flowgate's save_if_version optimistic locking | Low | Low |
