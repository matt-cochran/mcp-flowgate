...
| `parallel` executor kind (SPEC §24) | Fan-out / fan-in inside one transition. Config shape (branches, join, max_concurrency, on_branch_failure, timeouts, max_recursion_depth), output shape (`{branches, summary}`), audit event taxonomy, and the §24.5 "fan-out inside one transition" invariant are committed. The join enum may grow (`percent`, `expression-based`) per §24.8 amendment criterion. Compensating transactions deferred to v2. |
