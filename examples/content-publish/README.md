...
see [`docs/MCP-CONTROL-ARCHITECTURE.md`](../../docs/MCP-CONTROL-ARCHITECTURE.md)).
...
```bash
mcp-flowgate check --config examples/content-publish/gateway.yaml
```
...
resolves the approval using the `mcp-flowgate approvals` subcommand:

```bash
# See what's waiting
mcp-flowgate approvals list --config examples/content-publish/gateway.yaml

# Approve
mcp-flowgate approvals resolve --config examples/content-publish/gateway.yaml \
