...
mcp-flowgate deployment where different humans share one gateway.
...
[docs/MCP-CONTROL-ARCHITECTURE.md](../../docs/MCP-CONTROL-ARCHITECTURE.md#identity-in-multi-tenant-deployments)
...
mcp-flowgate serve --config examples/multi-tenant/gateway.yaml \
...
```bash
# Validate the config
cargo run -p mcp-flowgate -- check --config examples/multi-tenant/gateway.yaml

# Serve (stdio — anonymous principal, all guards pass)
cargo run -p mcp-flowgate -- serve --config examples/multi-tenant/gateway.yaml
```
...
[`crates/mcp-flowgate-core/tests/multi_tenant.rs`](../../crates/mcp-flowgate-core/tests/multi_tenant.rs).
...
```bash
cargo test -p mcp-flowgate-core --test multi_tenant
```

## See also

- [MCP-CONTROL-ARCHITECTURE.md](../../docs/MCP-CONTROL-ARCHITECTURE.md#identity-in-multi-tenant-deployments)
- [GOVERNANCE.md](../../docs/GOVERNANCE.md#guards-preconditions)
- [CONFIG.md](../../docs/CONFIG.md#capabilities)