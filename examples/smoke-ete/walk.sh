...
cargo run --quiet --bin mcp-flowgate -- check --config examples/smoke-ete/gateway.yaml

echo
echo "[2/3] Run ete_smoke integration test..."
cargo test --quiet -p mcp-flowgate-executors --test ete_smoke
...
echo
echo "Next: examples/smoke-ete/walk-live.sh (needs ANTHROPIC_API_KEY) for live-LLM coverage."
