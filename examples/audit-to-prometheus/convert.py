#!/usr/bin/env python3
"""
Convert mcp-flowgate audit JSONL to Prometheus metrics.

Reads audit events from stdin (one JSON line per event) and emits
Prometheus exposition-format metrics to stdout.

Usage:
  tail -f /var/log/flowgate-audit.jsonl | python3 convert.py

Or as a one-shot:
  cat /var/log/flowgate-audit.jsonl | python3 convert.py
"""
...
METRIC_PREFIX = "mcp_flowgate_audit"
...
def main():
    print(
        f"# mcp-flowgate audit metrics (generated {datetime.now(timezone.utc).isoformat()})"
    )
