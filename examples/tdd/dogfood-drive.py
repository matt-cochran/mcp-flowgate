...
Drives `mcp-flowgate serve` over stdio (newline-delimited JSON-RPC),
...
Run from the workspace root after `cargo build --release -p mcp-flowgate`:

    python3 examples/tdd/dogfood-drive.py

Expected: initialize handshake succeeds, tools/list returns exactly the
seven stable tools, workflow.start returns a workflow id + a link to
`start_cycle`, and submitting `start_cycle` advances to `red_pending`
with a baseline count matching this repo's current test count.
"""
import json
import os
import subprocess
...
GATEWAY = "./target/release/mcp-flowgate"
CONFIG = "examples/tdd/gateway.yaml"

if not os.path.exists(GATEWAY):
    sys.exit(f"binary not found at {GATEWAY} — run `cargo build --release -p mcp-flowgate` first")
...
def recv_response(expect_id, timeout=60.0):
    ...
    Finding from this dogfood: when the gateway has `audit.sink: stdout`
...
try:
    ...
    # 2. tools/list — should be exactly the seven stable tools.
    list_resp = step("tools/list", {
        "jsonrpc": "2.0", "id": 2, "method": "tools/list", "params": {},
    })
    tools = [t["name"] for t in list_resp["result"]["tools"]]
    print("Tools:", tools)
    expected = {"gateway.home", "gateway.search", "gateway.describe",
    ...
    print("  ✓ exactly the seven stable tools")

    # 3. Start the TDD workflow with this repo's test commands.
    start_resp = step("workflow.start (tdd)", {
    ...
    submit_resp = step("workflow.submit (start_cycle)", {
    ...
    print("\n=== DOGFOOD SUMMARY ===")
    print(f"  Build:         binary exists, mcp-flowgate check passed")
    print(f"  Protocol:      initialize + tools/list + 2× tools/call all succeeded")
    print(f"  Invariant 9:   tool surface is exactly the seven stable tools")
