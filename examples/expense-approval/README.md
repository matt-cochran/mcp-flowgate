...
`permission:` field. The bundled `FlowgateServer` always presents
`Principal::anonymous()`, which has no permissions, so every approval
guard rejects. To run this example end-to-end, you need to:

1. Build a custom `ServerHandler` (per [`docs/EMBEDDING.md §8d`](../../docs/EMBEDDING.md#8d-identity-wiring-principal-into-a-custom-server-surface))
...
```bash
mcp-flowgate check --config examples/expense-approval/gateway.yaml
```
...
- [`../../docs/MCP-CONTROL-ARCHITECTURE.md`](../../docs/MCP-CONTROL-ARCHITECTURE.md)
  — multi-tenant deployment patterns and the identity discussion
- [`../../docs/EMBEDDING.md`](../../docs/EMBEDDING.md#8d-identity-wiring-principal-into-a-custom-server-surface)
  — concrete `Principal` sourcing recipes
- [`../../docs/GOVERNANCE.md`](../../docs/GOVERNANCE.md) — guards,
