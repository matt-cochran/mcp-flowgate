...
merged and deduplicated, and a human arbitrates. Built entirely on Flowgate
primitives — no custom orchestrator, no agent loop, no scheduler.
...
```bash
# Validate
mcp-flowgate check --config examples/ai-review-swarm/gateway.yaml

# Run (gateway + TUI)
mcp-flowgate serve --config examples/ai-review-swarm/gateway.yaml
```
